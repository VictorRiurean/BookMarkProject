        ...
        content
            ...
            .onAppear {
                ...
                if !theme.isThemePreviouslySet {
                    theme.applySet(set: colorScheme == .dark ? .bpDark : .bpLight)
                    
                    theme.isThemePreviouslySet = true
                } else if theme.followSystemColorScheme,
                          theme.isThemePreviouslySet,
                          let sets = availableColorsSets.first(
                            where: { $0.light.name == theme.selectedSet || $0.dark.name == theme.selectedSet }
                          ) {
